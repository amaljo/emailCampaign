...
class Actions extends CI_Controller {
    ...
    function subscribe() {
        ...
        if ($this->input->post('subscribe')) {
            ...
            $this->form_validation->set_rules('clientId', 'Client Id', 'trim|required|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE || empty($clientDetails)) {
                ...
                if (empty($subcription)) {
                    ...
                    $this->email->message($this->load->view('mailTemplates/newsletterSubscribe', array_merge($data, $clientDetails), true));
                    $this->email->send();
                } else {
                    $response['message'] = 'Al ready subscribed';
                }
            }
        }
